...
SMALL_TEXT = """“Oh come on, you had me until…” My mother waved her hand.
...
def test_open_file():
    assert'“So I’m supposed to just – abandon her, in her hour of need?”' in  translate.open_file('../data/example.txt')
...
def test_split_content():
    ...
    '''print(TEXT)
    print(whole_text)
    assert whole_text == TEXT'''

    chunks = translate._split_content(SMALL_TEXT, '', chars_per_chunk=100)
    assert len(chunks) == 3
    whole_text = ''.join(chunks)
    print(SMALL_TEXT)
    print(len(SMALL_TEXT))
    print(len(whole_text))
    assert SMALL_TEXT == whole_text
